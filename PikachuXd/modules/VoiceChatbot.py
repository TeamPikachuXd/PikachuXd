...
from PikachuXd.services.pyrogram import pbot as LYCIA
...
@LYCIA.on_message(filters.command("PikachuXd"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("PikachuXd AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("PikachuXd Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=PikachuXd&ownername=TeamPikachuXd&user=1"
        )
        chatbot = L["message"]
