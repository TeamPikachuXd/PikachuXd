...
from PikachuXd.services.mongo import mongodb as db
...
async def _(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "PikachuXd found some torrents for you. Take a look 👇",
        buttons=[
            [
                Button.inline(
...
async def paginate_prevtorrent(event):
    ...
    try:
        ...
        await event.reply("Sorry, PikachuXd Cant found any torrents for that word")
        print(e)
        return
    vector = len(results)
    if num < 0:
...
async def torrentstop(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "Thanks for using.\n❤️ from [PikachuXd](t.me/PikachuXdBot) !",
        link_preview=False,
    )
